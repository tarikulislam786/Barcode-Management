...
//            $return_arr[] =  $row['notes'];
    ...
    try {
        ...
        $stmt = $conn->prepare('SELECT barcode, subject,formula,characteristics, category, steps, warningMark, procedures, notes FROM barcodeDetails WHERE barcode='.$barcode);
...
//echo $return_arr;
    ...
    /* Toss back results as json encoded array. */
    echo json_encode(array('barcode'=>$return_arr['barcode'],'subject'=>$return_arr['subject'], 'formula'=>$return_arr['formula'], 'characteristics'=>$return_arr['characteristics'], 'category'=>$return_arr['category'],'steps'=>$return_arr['steps'],'warningMark'=>$return_arr['warningMark'],'procedures'=>$return_arr['procedures'],'notes'=>$return_arr['notes']));
//echo json_encode($return_arr);
}
?>